...
@click.option(
    "--date",
    required=True,
    help="Enter date from which the commits are to be considered (YYYY-MM-DD)",
    prompt="Enter date from which the commits are to be considered (YYYY-MM-DD)",
)
@click.option(
    "--author",
    is_flag=True,
    help="Stats for a particular author?",
    prompt="Stats for a particular author?",
)
def git_stats(date, author):
    ...
    click.echo(
        "{} {} changed, {} insertions(+), {} deletions(-)".format(
            files, "file" if files == 1 else "files", insertions, deletions
        )
    )
    os.remove(".git.log")
