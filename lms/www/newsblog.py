...
def get_context(context):
    ...
    context.related_articles = frappe.db.sql(
        "select nb.title, nb.name, DATE_FORMAT(nb.publishing_date, %(format)s) as publishing_date, GROUP_CONCAT(bt.website_tags) as website_tags, nb.for_banner_view from `tabNews and Blog` as nb left join `tabBlog Tags` bt on (bt.parent = nb.name) where nb.name in (Select nb.name from `tabNews and Blog` nb, `tabBlog Tags` bt where bt.parent=nb.name AND nb.name <> '{}' AND bt.website_tags in {} group by nb.name) group by nb.name order by nb.creation desc limit 3;".format(
