	...
	def check_for_shortfall(self):
		...
		if check:
			...
			if customer.alerts_based_on_percentage:
				if self.total_collateral_value > (old_total_collateral_value+(old_total_collateral_value*int(customer.alerts_based_on_percentage)/100)):
					try:
						fa = FirebaseAdmin()
						fa.send_data(
							data={
								"event": "Alert price UP by {}%".format(customer.alerts_based_on_percentage),
							},
				...
				elif self.total_collateral_value < (old_total_collateral_value-(old_total_collateral_value*customer.alerts_based_on_percentage/100)):
					try:
						fa = FirebaseAdmin()
						fa.send_data(
							data={
								"event": "Alert price DOWN by {}%".format(customer.alerts_based_on_percentage),
							},
							tokens=lms.get_firebase_tokens(customer.user),
						)
					except Exception:
				...
				if self.total_collateral_value > (old_total_collateral_value+customer.alerts_based_on_amount):
					try:
						fa = FirebaseAdmin()
						fa.send_data(
							data={
								"event": "Alert price UP by Rs. {}".format(customer.alerts_based_on_amount),
							},
							tokens=lms.get_firebase_tokens(customer.user),
						)
					except Exception:
				...
				elif self.total_collateral_value < (old_total_collateral_value-customer.alerts_based_on_amount):
					try:
						fa = FirebaseAdmin()
						fa.send_data(
							data={
								"event": "Alert price DOWN by Rs. {}".format(customer.alerts_based_on_amount),
							},
							tokens=lms.get_firebase_tokens(customer.user),
						)
					except Exception:
