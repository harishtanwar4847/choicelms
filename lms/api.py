    ...
    try:
        ...
        if res == True:
            ...
            return generateResponse(
                status=500,
                message="There was some problem while sending OTP. Please try again.",
            )
...
def register_customer(first_name, last_name, phone, email, otp):
    try:
        ...
        otpobj = frappe.db.get_all(
            "Mobile OTP", {"mobile_no": phone, "otp": otp, "verified": 0}
        )
        if len(otpobj) == 0:
        ...
        if type(user_name) is str:
            ...
            return generateResponse(
                status=500, message="Something Wrong During User Creation. Try Again."
            )
...
def send_otp(phone):
    try:
        # delete unused otp
        frappe.db.delete("Mobile OTP", {"mobile_no": phone, "verified": 0})
        ...
        otp_doc = frappe.get_doc(
            dict(doctype="Mobile OTP", mobile_no=phone, otp=OTP_CODE)
        ).insert(ignore_permissions=True)
...
def save_user_kyc(*args, **kwargs):
    ...
    if data.get("accountNumber", None):
        user_bank_details_doc = frappe.get_doc(
            dict(
...
def get_user_kyc(pan_no, birth_date):
    try:
        ...
        user_kyc_list = frappe.db.get_all(
            "User KYC",
        ...
        params = {
            "inputXML": "<APP_REQ_ROOT><APP_PAN_INQ> <APP_PAN_NO>{0}</APP_PAN_NO> <APP_DOB_INCORP>{1}</APP_DOB_INCORP> <APP_POS_CODE>1100066900</APP_POS_CODE> <APP_RTA_CODE>1100066900</APP_RTA_CODE> <APP_KRA_CODE></APP_KRA_CODE> <FETCH_TYPE>I</FETCH_TYPE> </APP_PAN_INQ></APP_REQ_ROOT>".format(
                pan_no, birth_date
            ),
            "userName": "EKYC",
            "PosCode": "1100066900",
            ...
            user_kyc = frappe.get_doc(
                {
                    ...
                    "address": ", ".join(
                        filter(
                            [
                                data["APP_PAN_INQ"]["APP_PER_ADD1"],
                                data["APP_PAN_INQ"]["APP_PER_ADD2"],
                                data["APP_PAN_INQ"]["APP_PER_ADD3"],
                            ],
                            None,
                        )
                    ),
...
def get_share_list():
    try:
        ...
        user_kyc_list = frappe.db.get_all(
            "User KYC",
            filters={"user_mobile_number": user.username},
            order_by="kyc_type",
            fields=["*"],
        )
        ...
        data = {"UserID": "Spark", "ClientID": user_kyc_list[0].choice_client_id}

        res = requests.post(url, json=data, headers={"Accept": "application/json"})
        if not res.ok:
            return generateResponse(
                status=res.status_code,
                message="There was a problem while getting share list from choice.",
            )
        res_json = res.json()
        if res_json["Status"] != "Success":
            return generateResponse(
                status=422, message="Problem in getting securities list"
            )

        # setting eligibility
        securities_list = res_json["Response"]
        for i in securities_list:
            allowed_securities_list = frappe.db.get_all(
                "Allowed Security", filters={"isin_no": i["ISIN"]}, fields=["*"]
            )
            i["Is_Eligible"] = False
            i["Category"] = None
...
def cdsl_pedge(securities, expiry=None, pledgor_boid=None, pledgee_boid=None):
    try:
        ...
        securities_array = []
        for i in securities:
            j = {"ISIN": i["isin"], "Quantity": i["quantity"], "Value": i["value"]}
        ...
        response = requests.post(API_URL, headers=get_cdsl_headers(), json=payload)
        ...
        if response_json and response_json.get("Success") == True:
            ...
            return generateResponse(
                is_success=False, message="CDSL Pledge Error", data=response_json
            )
    except Exception as e:
        print(e)
        return generateResponse(is_success=False, error=e)
...
def cdsl_unpedge(securities_array=None):
    try:
        if not securities_array:
            securities_array = [{"PSNumber": "1930499", "PartQuantity": "100.000"}]
...
def cdsl_confiscate(securities_array=None):
    try:
        ...
        if response_json and response_json.get("Success") == True:
            ...
            return generateResponse(
                is_success=False, message="CDSL Confiscate Error", data=response_json
            )
    except Exception as e:
        print(e)
        return generateResponse(
