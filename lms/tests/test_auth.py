...
class Login(utils.APITestCase):
    ...
    def tearDownClass(cls):
        frappe.db.delete("User Token", {"entity": cls.mobile})
...
class VerifyOTP(utils.APITestCase):
    ...
    def test_user_not_found(self):
        otp = lms.create_user_token(
            entity=self.mobile, token=lms.random_token(length=4, is_numeric=True)
        )
        res = self.client.post_api(
            "lms.auth.verify_otp",
            {"otp": otp.token, "firebase_token": "asdf", "mobile": self.mobile},
        )
    ...
    @classmethod
    def tearDownClass(cls):
        frappe.db.delete("User", {"username": cls.mobile})
        frappe.db.delete("User Token", {"entity": cls.mobile})
...
class Register(utils.APITestCase):
    ...
    def test_email_exist(self):
        res = self.client.post_api(
            "lms.auth.register",
            {
                ...
                "last_name": self.last_name,
                "mobile": self.mobile,
                "email": "iccha.konalkar12@atritechnocrat.in",
            },
        )
        self.assertValidationError(res)
    ...
    def test_register_success(self):
        res = self.client.post_api(
            "lms.auth.register",
            {
                "firebase_token": "cSQk2fGpS76sEaCiuSnoss:APA91bFK2wlTBuH6Xi5Di5w4QJpd0hithe8z2y43eVs7kAliac8eRhaAjY-cSQk2fGpS76sEaCiuSnoss:APA91bFK2wlTBuH6Xi5Di5w4QJpd0hithe8z2y43eVs7kAliac8eRhaAjY-AFmhJmWQ454rl9W6Zr85c4ziX76_ymAj8617Zu2PGJWBM347pfMmW_zOMoAQ1d0ylyWmY2e03W5AHDLCO",
                "first_name": self.first_name,
                "last_name": self.last_name,
                "mobile": self.mobile,
                "email": self.email,
    ...
    @classmethod
    def tearDownClass(cls):
        frappe.db.delete("User", {"name": cls.email})
        frappe.db.delete("User Token", {"entity": cls.mobile})
