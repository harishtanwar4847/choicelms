...
def get_profile_set_alerts(**kwargs):
	try:
		...
		elif data.get("is_for_alerts") and data.get("percentage") and data.get("amount"):
			return utils.respondWithFailure(status=417, message=frappe._("Please choose one between Amount or Percentage for setting Alerts"))
...
def check_eligible_limit(**kwargs):
	try:
		...
		data = utils.validator.validate(kwargs, {"lender": ""})
		if not data.get("lender"):
			data["lender"] = frappe.get_last_doc("Lender").name
