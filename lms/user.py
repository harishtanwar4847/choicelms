...
def kyc(pan_no=None, birth_date=None):
	try:
		...
		user_kyc_list = frappe.db.get_all("User KYC", filters={ "user_mobile_number": user.username, "pan_no": pan_no }, order_by="kyc_type", fields=["*"])

		if len(user_kyc_list) > 0:
			return lms.generateResponse(message="User KYC", data=user_kyc_list[0])
		else:
			return lms.generateResponse(message="Enter Valid Pan No")
