...
def kyc_old(pan_no=None, birth_date=None):
    try:
        ...
        user_kyc_list = frappe.db.get_all(
            "User KYC",
            filters={"pan_no": pan_no, "user_mobile_number": user.username},
            order_by="kyc_type",
            fields=["*"],
        )
        ...
        if not r.ok:
            return lms.generateResponse(
                ...
                message=_("A problem occured while fetching KYC details from CHOICE."),
            )
...
def esign_old(cart_name=None, loan_name=None):
    try:
        ...
        if entity.customer != customer.name:
            return lms.generateResponse(
                status=403, message=_("Please use your own cart/loan.")
            )
        ...
        r = requests.post(
            las_settings.esign_upload_file_url, files=files, headers=headers
        )

        if not r.ok:
            return lms.generateResponse(
                status=500,
                message=_("There was some problem in uploading loan aggrement file"),
            )

        data = r.json()

        url = las_settings.esign_request_url.format(
            id=data.get("id"), x=200, y=200, page_number=2
        )

        return lms.generateResponse(
            message=_("Esign URL."), data={"esign_url": url, "file_id": data.get("id")}
        )
    except (lms.ValidationError, lms.ServerError) as e:
        return lms.generateResponse(status=e.http_status_code, message=str(e))
    except Exception as e:
    ...
    try:
        ...
        if not res.ok:
            return lms.generateResponse(
                ...
                message=_("There was a problem while getting share list from choice."),
            )
        res_json = res.json()
        if res_json["Status"] != "Success":
            return lms.generateResponse(
                status=422, message=_("Problem in getting securities list.")
            )
...
def tds(tds_amount, year):
    ...
    return lms.generateResponse(
        message=_("TDS Create Successfully."), data={"file": tds}
    )
