...
"""Concentration rule - securities api  - 04-03-2022 """
...
def securities(**kwargs):
    try:
        ...
        data = utils.validator.validate(
            kwargs,
            {"lender": "", "level": "", "pledgor_boid": ""},
        )

        reg = lms.regex_special_characters(search=data.get("pledgor_boid"))
        if reg:
            return utils.respondWithFailure(
                status=422,
                message=frappe._("Special Characters not allowed."),
        ...
        if not data.get("lender", None):
            ...
            lender_list = data.get("lender", None)["list"]
        ...
        securities_list_demat_filtered = []
        securities_list_level_filtered = []
        ...
        for i in securities_list:
            ...
            try:
                ...
                i["Allowed_Lenders"] = securities_category_map[i["ISIN"]].get("lender")
                i["Is_Eligible"] = True
                i["Total_Qty"] = i["Quantity"]
            ...
            if data.get("pledgor_boid"):
                demat_ = data.get("pledgor_boid")
                if i["Stock_At"] == demat_:
                    securities_list_demat_filtered.append(i)
                i["Category"] = lms.update_security_category(i["Category"])
                data_security = securities_list_demat_filtered
            else:
                i["Category"] = lms.update_security_category(i["Category"])
                data_security = securities_list

        if data.get("level"):
            security_categories = []
            for lender in lender_list:
                lender_doc = frappe.get_doc("Lender", lender)
                for level in data.get("level")["list"]:
                    if lender_doc.concentration_rule:
                        security_categories.append(
                            lender_doc.concentration_rule[
                                int(level) - 1
                            ].security_category
                        )
            for i in securities_list:
                if i["Category"] in security_categories:
                    securities_list_level_filtered.append(i)
                    i["Category"] = lms.update_security_category(i["Category"])
            data_security = securities_list_level_filtered
        return utils.respondWithSuccess(data=data_security)
...
def securities_new(**kwargs):
    try:
        ...
        securities_exists = frappe.db.exists(
            {
                "doctype": "Client Holding",
                "pan": user_kyc.pan_no,
                "creation": datetime.strftime(frappe.utils.now_datetime(), "%Y-%m-%d"),
            }
        )
        print(securities_exists)
        securities_list = frappe.db.sql(
            """
        SELECT `pan` as PAN, `isin` as ISIN, `branch` as Branch, `client_code` as Client_Code, `client_name` as Client_Name, `scrip_name` as Scrip_Name, `depository` as Depository, `stock_at` as Stock_At, `quantity` as Quantity, `price` as Price, `scrip_value` as Scrip_Value, `holding_as_on` as Holding_As_On
        FROM `tabClient Holding` as ch
