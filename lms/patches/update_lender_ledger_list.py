...
def execute():
    ...
    for lender_ledger in lender_ledgers:
        ...
        """.format(lender_ledger.loan))
            frappe.db.sql("""update `tabLender Ledger` set transaction_type = (select transaction_type from `tabLoan Transaction` where name = '{}')
        """.format(lender_ledger.loan_transaction))
    frappe.db.commit()
