...
def validate_securities_for_cart(securities, lender):
    if not securities or (
        ...
        raise utils.ValidationException(
            {"securities": {"required": _("Securities required.")}}
        )
...
def upsert_old(securities, cart_name=None, loan_name=None, expiry=None):
    try:
        ...
        if securities_valid:
            securities_list_from_db_ = frappe.db.sql(
                "select isin from `tabAllowed Security` where isin in {}".format(
        ...
        if not cart_name:
            cart = frappe.get_doc({"doctype": "Cart", "customer": customer.name})
            for i in securities:
                cart.append(
                    "items",
                    {
            ...
            if cart.customer != customer.name:
                return lms.generateResponse(
                    status=403, message=_("Please use your own cart.")
                )
        ...
        if loan_name:
            ...
            if not loan_margin_shortfall.get("__islocal", 0):
                data = {
                    "cart": data,
                    "minimum_pledge_amount_present": True,
                    "minimum_pledge_amount": loan_margin_shortfall.shortfall_c * 2,
                }
...
@frappe.whitelist()
def process_old(
    cart_name, otp, pledgor_boid, file_id=None, expiry=None, pledgee_boid=None
):
    try:
        # validation
        lms.validate_http_method("POST")
        ...
        otp_res = lms.check_user_token(
            entity=frappe.session.user, token=otp, token_type="Pledge OTP"
        )
        if not otp_res[0]:
        ...
        frappe.logger().info(
            {
                "CDSL PLEDGE HEADERS": las_settings.cdsl_headers(),
                "CDSL PLEDGE PAYLOAD": payload,
                "CDSL PLEDGE RESPONSE": response_json,
            }
        )
        ...
        if response.ok and response_json.get("Success") == True:
            ...
            for key, group in groupby(
                response_json["PledgeSetupResponse"]["ISINstatusDtls"],
                key=lambda x: x["ISIN"],
            ):
            ...
            for item in cart.items:
                frappe.logger().info(
                    {
                        "print error code": response_json_item_groups[item.isin][
                            "ErrorCode"
                        ]
                    }
                )
            ...
            if file_id:
                lms.save_signed_document(
                    file_id, doctype="Loan Application", docname=loan_application.name
                )
