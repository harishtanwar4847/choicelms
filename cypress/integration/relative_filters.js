context("Relative Timeframe", () => {
  ...
  it("set relative filter for Previous and check list", () => {
    ...
    cy.get(".fieldname-select-area input").type("Due Date{enter}", {
      delay: 100,
    });
    cy.get("select.condition.form-control").select("Previous");
    cy.get(".filter-field select.input-with-feedback.form-control").select(
      "1 week"
    );
    cy.server();
    cy.route("POST", "/api/method/frappe.desk.reportview.get").as(
      "list_refresh"
    );
    ...
    cy.route("POST", "/api/method/frappe.model.utils.user_settings.save").as(
      "save_user_settings"
    );
